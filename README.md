# Bo-Cheng Shih 👋
### National Chengchi University (NCCU), Taiwan — BA in Diplomacy Candidate
### 國立政治大學外交系

## 🎯 研究領域 | Research Focus

### 核心關注 | Core Interests
* **核不擴散理論、延伸嚇阻與美中(中俄)戰略互動**
  * *Nuclear Non-proliferation Theory, Extended Deterrence, and U.S.–China(-Russia) Strategic Interaction*
* 主要探討大國武器移轉的政治邏輯及其對地區安全秩序的影響，特別聚焦於美國對亞洲盟邦的軍售模式與中國威脅感知之間的關聯性。
  * *My work examines the political logic of great-power arms transfers and their effects on regional security order, with a focus on the relationship between U.S. arms sales to Asian allies and China's threat perception.*

### 議題 | Issues
* Nuclear Deterrence and Extended Deterrence
* Strategic Stability
* Reputation & Credibility
* Multipolarity / Tripolar Competition

### 區域 | Region
* China and Russia

---

## 🧩 理論框架 | Theoretical Framework

### `Asymmetric Threat Resonance (ATR)`
* 探討中俄在缺乏明確協調的情況下，如何透過核訊號的相互強化侵蝕中國「不首先使用」政策的可信度，進而削弱美國在印太地區的延伸嚇阻。
  * *Explores how China and Russia, without explicit coordination, produce mutually reinforcing nuclear signals that erode the credibility of China's No First Use policy and undermine U.S. extended deterrence in the Indo-Pacific.*
* 理論脈絡承接 `Waltz–Sagan` 核擴散辯論、`Yarhi-Milo`、`Lanoszka & Cooper` 的 **「Patron's Dilemma」**。
  * *Builds on the Waltz–Sagan proliferation debate and Yarhi-Milo, Lanoszka & Cooper's "Patron's Dilemma."*

---

## 🔬 目前研究 | Current Project

### 國科會大專生研究計畫 | Taiwan NSTC Undergraduate Research Grant
* 以美國對亞洲盟邦軍售與中國威脅感知的量化關聯性為主題，採 `panel data fixed-effect` 方法，輔以 Taiwan / Japan / Philippines 小樣本比較作為備援設計。
  * *Quantitative study of the relationship between U.S. arms sales to Asian allies and Chinese threat perception, using a panel data fixed-effects approach with a small-N comparison (Taiwan, Japan, Philippines) as a backup design.*

---

## 🧰 工具 | Tools
* **資料分析 Data Analysis:** `R`（panel data / fixed-effect models）
* **文獻管理 Literature:** `Notion`
* **版本控制 Version Control:** `GitHub`

---

## 📫 聯絡方式 | Contact
* **Email:** [114203046@g.nccu.edu.tw](mailto:114203046@g.nccu.edu.tw) / [bocheng1227@gmail.com](mailto:bocheng1227@gmail.com)
* **Medium:** [https://medium.com/@bocheng1227](https://medium.com/@bocheng1227)

***

> 🕒 *Last updated: 2026年7月*
